...
#define STREAM_DURATION   10.0  //流时长 单位秒
#define STREAM_FRAME_RATE 25 /* 25 images/s */
#define STREAM_PIX_FMT   AV_PIX_FMT_NV12 //AV_PIX_FMT_YUV420P /* default pix_fmt */
    ...
    AVCodec** codec,enum AVCodecID codec_id)
{
    ...
    *codec = avcodec_find_encoder(codec_id);    //通过codec_id找到编码器
    ...
    /* 为什么要 -1呢？每次调用avformat_new_stream的时候nb_streams+1
       但id是从0开始, 比如第1个流：对应流id = oc->nb_streams -1 = 0
                        第2个流：对应流id = oc->nb_streams -1 = 1
    */
...
static int write_audio_frame(AVFormatContext* oc, OutputStream* ost)
{
    ...
    if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF)
    {
        ...
        fprintf(stderr, "Error while writing video frame avcodec_receive_packet : %s\n", av_err2str(ret));
        exit(1);
    }
...
static void open_video(AVFormatContext* oc, AVCodec* codec, OutputStream* ost, AVDictionary* opt_arg)
{
    ...
    if (codec->id == AV_CODEC_ID_H264) {
        ...
        int ret = av_opt_set(codec_ctx->priv_data, "preset", "medium", 0);
        if (ret != 0) {
...
static void readyWrite(OutputStream* ost) {
    ...
    FILE* in_yuv_file = "E:/debug.yuv";      // 输入YUV文件
    g_infile = fopen(in_yuv_file, "rb");
    if (!g_infile) {
    ...
    // 计算出每一帧的数据 像素格式 * 宽 * 高
    // 1382400
    g_frame_bytes = av_image_get_buffer_size(frame->format, frame->width,
        frame->height, 1);
    printf("frame_bytes %d\n", g_frame_bytes);
    g_yuv_buf = (uint8_t*)malloc(g_frame_bytes);
    if (!g_yuv_buf) {
        printf("yuv_buf malloc failed\n");
        return 1;
    }
}
static AVFrame* read_video_frame(OutputStream* ost)
{

   memset(g_yuv_buf, 0, g_frame_bytes);
   size_t read_bytes = fread(g_yuv_buf, 1, g_frame_bytes, g_infile);
   if (read_bytes <= 0) {
       printf("read file finish\n");
       return NULL;
   }

   int need_size = av_image_fill_arrays(ost->frame->data, ost->frame->linesize, g_yuv_buf,
       ost->frame->format,
       ost->frame->width, ost->frame->height, 1);
   if (need_size != g_frame_bytes) {
       printf("av_image_fill_arrays failed, need_size:%d, frame_bytes:%d\n",
           need_size, g_frame_bytes);
       return NULL;
   }
...
static int write_video_frame(AVFormatContext* oc, OutputStream* ost)
{
    ...
    //frame = get_video_frame(ost);   //自动生成帧数据
    frame = read_video_frame(ost);
