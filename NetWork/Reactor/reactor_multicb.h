...
struct reactor* getInstance(void) { //singleton
	...
	if (instance == NULL) {
		...
		instance = (reactor*)malloc(sizeof(struct reactor));
...
int nreactor_set_event(int fd, NCALLBACK cb, int event, void* arg) {
	...
	if (event == READ_CB) {
		...
		/*r->head->items[fd].acceptcb = accept_callback;
		r->head->items[fd].readcb = read_callback;
		r->head->items[fd].writecb = write_callback;*/
		r->head->items[fd].arg = arg;
...
int init_server(int port) {
	...
	if (listen(listenfd, 10) == -1) {
		...
		printf("listen socket successed\n");
	}
	return listenfd;
...
int init_reactor(struct reactor* r) {
	...
	r->head->items = (nitem*)malloc(MAX_EPOLL_EVENT * sizeof(struct nitem));
